...
|---|---|---|
| Regra de saldo e atomicidade | implementado | transação única e lock pessimista por conta |
...
Já coberto: crédito, débito, saldo insuficiente, saldo exato, conta inexistente/desabilitada, moeda divergente, valores inválidos, duplicidade do evento, timestamps, REST, SQS, JPA e E2E.

Antes do go-live, adicionar:

- teste concorrente com muitas autorizações na mesma conta provando ausência de lost update/overdraft;
- idempotência concorrente e replay após timeout/commit;
- property-based tests para sequências aleatórias de crédito/débito;
- limites de `NUMERIC(19,2)` e overflow do saldo;
