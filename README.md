...
Serviço de autorização de crédito e débito construído com Java 21, Spring Boot, PostgreSQL e AWS SQS, organizado em DDD e arquitetura hexagonal. Créditos incrementam o saldo; débitos sem fundos são registrados como `FAILED` sem alterar a conta.
...
Os testes incluem domínio, aplicação, REST, mensageria, persistência com PostgreSQL real e fluxos E2E com PostgreSQL e LocalStack. O `verify` também gera `target/site/jacoco/jacoco.xml` para o SonarQube.

## Coleção de requisições

Importe no Postman:

- [coleção](postman/financial-transaction-authorization.postman_collection.json);
- [environment local](postman/local.postman_environment.json).

Envie primeiro `Create account event`, aguarde o consumo do SQS e execute os cenários de crédito e débito.

## Configuração

O profile default é `local`. Em produção, defina `SPRING_PROFILES_ACTIVE=production` para usar a cadeia de credenciais da workload em vez de credenciais estáticas do LocalStack. As principais variáveis são:
...
Segredos devem vir do secret manager da plataforma. O número de threads, conexões e mensagens em voo precisa ser dimensionado com teste de carga e com os limites globais do banco, nunca multiplicado livremente pelo autoscaling.
...
A API ainda precisa de idempotência de requisição para impedir crédito duplicado quando o cliente repetir uma chamada após timeout. Autenticação/autorização, infraestrutura como código e testes de carga também dependem do ambiente de destino. Esses itens estão priorizados e detalhados na documentação de production readiness; não devem ser tratados como concluídos apenas pela presença da pipeline e do container.
